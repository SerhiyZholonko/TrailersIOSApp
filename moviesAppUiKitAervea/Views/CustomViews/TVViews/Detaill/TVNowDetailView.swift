...
protocol TVNowDetailViewDeligate: AnyObject {
    func toreviewController(movieId: String)
    func toVideo(id: String)
}

class TVNowDetailView: UIView {
    //MARK: - Properties
    weak var delegate: TVNowDetailViewDeligate?
    ...
    lazy var playButton: UIButton = {
        ...
        button.addTarget(self, action: #selector(didVideoButomTap), for: .touchUpInside)
        return button
    }()
    lazy var infoView: InfoView = {
        let InfoView = InfoView(frame: .zero, rate: self.viewModel.rate, voteCount: viewModel.voteCount, garne: [])
    ...
    //MARK: - init
    init(frame: CGRect, viewModel: TVNowDetaillViewModel) {
   ...
   private func add(chaildVC: UIViewController, to containerView: UIView) {
    ...
    //MARK: - selector
   @objc private func didVideoButomTap(){
       delegate?.toVideo(id: "\(viewModel.movieId)")
    }
 }


//MARK: - ex ReactionViewDalegate

extension TVNowDetailView: ReactionViewDalegate {
